...
通过 YAML 声明式定义数据处理任务，支持多种任务类型：

```yaml
# 任务定义示例
namespace: "com.company.tasks"
name: "data_processor"
type: RAY_OPERATOR                    # 任务类型
description: "数据处理任务"
...
tasks:
  ...
  - id: "clean"
    taskRef:
      ...
      version: "1.0.0"
    inputs:
      input_path: "${pipeline.raw_data_path}"
      output_path: "${pipeline.output_base_path}/cleaned/"
      min_quality_score: "${pipeline.quality_threshold}"
  ...
  - id: "convert"
    ...
    inputs:
      input_path: "${clean.output_path}"
      output_path: "${pipeline.output_base_path}/final/"
