...
本文档汇总了 **ControlFlow 控制流编排引擎** 中的核心术语及其定义。所有术语在文档中以 **《术语 Term》** 格式标识。
...
**定义**: 任务类型的元定义，定义了"这类任务能做什么"。描述一类任务的能力边界，包括支持的行为、产生的事件、可能的状态、以及如何与执行器交互。

**职责**:
- 定义任务类型支持哪些行为（actions）
- 定义任务会产生哪些事件（events）
- 定义任务有哪些状态（states），支持自定义状态
- 定义执行器配置（executor）
- 通过 JSON Schema 约束执行配置的结构
- 支持用户注册自定义任务类型

**关键特性**:
- 完全可扩展：用户可以注册自定义 TaskSchema
- 自定义状态：支持轮询查询执行器的自定义状态
- 配置驱动：通过 YAML/JSON 配置，无需编写代码
- 内置类型：系统预置常用类型（pyspark, sql, streaming 等）
...
**定义**: 可复用的任务模板，是 **TaskSchema 的具体实例**。定义了"这个任务怎么做"，包括具体的执行配置和输入输出变量。独立于流水线存在，可被多个流水线的多个节点引用。

**职责**:
- 引用一个 TaskSchema (taskType)
- 提供符合 Schema 要求的具体执行配置
- 声明输入变量和输出变量接口契约
- 支持版本管理和跨流水线复用

**与 TaskSchema 的关系**:
- TaskSchema 定义"能做什么"（能力边界）
- TaskDefinition 定义"怎么做"（具体配置）
- 一个 Schema 可以有多个 Definition

**相关术语**: **《TaskSchema 任务模式》**、**《Node 节点》**、**《Pipeline 流水线》**
...
**定义**: 任务类型的字符串标识符，用于引用 TaskSchema。在新设计中，TaskType 不再是枚举，而是 TaskSchema 的 `type` 字段。

**常见类型**（内置 TaskSchema）:
- **pyspark_operator**: PySpark 批处理任务
- **sql_operator**: SQL 查询任务
- **ray_operator**: Ray 分布式计算任务
- **streaming_operator**: 流处理任务
- **approval**: 审批任务
- **model_inference**: 模型推理任务

**用户自定义类型**:
- 用户可以注册任意 taskType，如：`custom_ml_training`, `data_quality_check` 等

**相关术语**: **《TaskSchema 任务模式》**、**《TaskDefinition 任务定义》**
...
- **《TaskSchema 任务模式》** - 任务类型的元定义
- **《TaskType 任务类型》** - 任务类型标识符
